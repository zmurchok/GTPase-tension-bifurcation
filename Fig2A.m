...
if loaddata==0
  ...
  opt = contset(opt,'MaxNumPoints',1000);%%INCREASE THIS FOR MORE LIMIT CYCLE
...
x = xlc;
s = slc;
plot(x(end,:),max(x((0:lds.tps-1)*lds.nphase+1,:)),'color',1-(1-green)*1.1,'linewidth',2)
plot(x(end,:),min(x((0:lds.tps-1)*lds.nphase+1,:)),'color',1-(1-green)*1.1,'linewidth',2)
for j = 2:size(x,2)-1 %don't do the whole curve becuase the starting point and end point shouldn't be flagged
  if ~isempty(find([s.index]==j))
    plot(x(end,j),max(x((0:lds.tps-1)*lds.nphase+1,j)),'color',black,'linewidth',3)
    plot(x(end,j),min(x((0:lds.tps-1)*lds.nphase+1,j)),'color',black,'linewidth',3)
  end
end
...
axis([0.05 0.3 0 1.2])
print(1,'Fig2A.eps','-depsc','-painters')
print(1,'Fig2A.png','-dpng','-r600')
