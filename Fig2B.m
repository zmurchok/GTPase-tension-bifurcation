...
%color limit cycle max/min by limit cycle stability... not sure how to do this automatically with matcont.
%xppaut does this... see Fig2B_xppaut.ps and GTPase-tension.ode for a plot...

%subcritical part
plot(x(end,1:s(2).index),max(x((0:lds.tps-1)*lds.nphase+1,1:s(2).index)),'color',1-(1-green)*0.8,'linewidth',1)
plot(x(end,1:s(2).index),min(x((0:lds.tps-1)*lds.nphase+1,1:s(2).index)),'color',1-(1-green)*0.8,'linewidth',1)
%stable part
plot(x(end,s(2).index:s(3).index),max(x((0:lds.tps-1)*lds.nphase+1,s(2).index:s(3).index)),'color',1-(1-green)*1.1,'linewidth',2)
plot(x(end,s(2).index:s(3).index),min(x((0:lds.tps-1)*lds.nphase+1,s(2).index:s(3).index)),'color',1-(1-green)*1.1,'linewidth',2)
%unstable part
plot(x(end,s(3).index:s(4).index),max(x((0:lds.tps-1)*lds.nphase+1,s(3).index:s(4).index)),'color',1-(1-green)*0.8,'linewidth',1)
plot(x(end,s(3).index:s(4).index),min(x((0:lds.tps-1)*lds.nphase+1,s(3).index:s(4).index)),'color',1-(1-green)*0.8,'linewidth',1)



% ADD DOTS FOR LPC POINTS on LIMIT CYCLE
...
axis([0.155 0.165 0.2 1])
print(1,'Fig2B.eps','-depsc','-painters')
print(1,'Fig2B.png','-dpng','-r600')
